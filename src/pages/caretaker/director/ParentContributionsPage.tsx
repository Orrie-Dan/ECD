...
export function ParentContributionsPage() {
  return (
    <CaretakerLayout
        ...
        <PageContainer>
          ...
          <PageContent>
            <LiveUnavailableState
              title={copy.mockOnlyTitle}
              description={copy.mockOnlyBody}
            />
          </PageContent>
        </PageContainer>
      ) : (
        <ParentContributionsLive />
      )}
    </CaretakerLayout>
  )
}

function ParentContributionsLive() {
  const { user } = useAuth()
  ...
  const [archiveTarget, setArchiveTarget] = useState<ParentContributionViewModel | null>(
    null,
  )
  ...
  const listFilters = useMemo(
    ...
    [centerId, range.from, range.to, page, pageSize, typeFilter],
  )
  ...
  const list = useParentContributionsList(listFilters, Boolean(centerId))
  const summary = useParentContributionSummary(summaryFilters, Boolean(centerId))
        ...
        >
          {summary.isLoading ? (
            ...
            <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
              <StatCard
                icon={<Users size={22} />}
                label={copy.summaryParents}
                value={parentsContributed}
              />
              <StatCard
