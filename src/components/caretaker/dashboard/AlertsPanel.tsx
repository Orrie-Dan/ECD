...
export function AlertsPanel({ centerId, limit = 4 }: AlertsPanelProps) {
  ...
  const mockAlerts = getHighPriorityAlerts()
    .filter((a) => !centerId || a.centerId === centerId)
    .slice(0, limit)
        ...
        {mockAlerts.map((alert) => (
          <li key={alert.id}>
            ...
            >
              ...
              />
              <div className="flex-1 min-w-0">
                <p className="text-body font-semibold text-text truncate">{alert.centerName}</p>
                <p className="text-caption text-warning mt-0.5 line-clamp-2">{alert.description}</p>
              </div>
              <ChevronRight
                size={16}
                className="text-text-muted shrink-0 mt-1 opacity-60 group-hover:opacity-100"
